...
class Trainer:
    def __init__(self, options):
        ...
        if self.opt.encoder=="resnet":
            ...
            self.models["depth"] = networks.VANDecoder(
                self.models["encoder"].num_ch_enc, self.opt.scales, self.opt.height, self.opt.width)
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                if self.opt.height_loss_weight>0 and i==0:
                    h = self.opt.height//(2**source_scale)
                    w = self.opt.width//(2**source_scale)
                    outputs[("3d_points", scale)] = cam_points[:,:-1,:].view(-1, 3, h, w)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.height_loss_weight>0:
                h = self.opt.height//(2**source_scale)
                w = self.opt.width//(2**source_scale)
                Points = outputs[("3d_points", scale)]
                proposal_region_points = (Points[:,:,int(self.opt.upper_rate*h):,int(w*self.opt.left_rate):int(w*self.opt.right_rate)]).clone().detach()
    ...
    def compute_height_loss(self, Points, proposal_region_points):
        ...
        if self.opt.disable_CPD and errors.min()>=self.opt.delta:
            ...
            #print(height_loss.mean(),1.0*height_loss.numel()/(b*h*w))
